...
function getStudent(id) {
  return db.students.find(s => s.id === Number(id));
}
...
function deleteStudent(id) {
  ...
  db.students = db.students.filter(s => s.id !== numericId);
  db.progress = db.progress.filter(p => Number(p.student_id) !== numericId);
  db.reflections = db.reflections.filter(r => Number(r.student_id) !== numericId);
  db.badges = db.badges.filter(b => Number(b.student_id) !== numericId);
  db.essays = db.essays.filter(e => Number(e.student_id) !== numericId);
  saveData();
}

function getAllStudents() {
  return db.students.map(s => {
    const s_prog = db.progress.filter(p => Number(p.student_id) === s.id);
    const s_badges = db.badges.filter(b => Number(b.student_id) === s.id);
    return {
      id: s.id,
      name: s.name,
      class: s.class,
...
function getStudentData(studentId) {
  ...
  return {
    student,
    progress: db.progress.filter(p => Number(p.student_id) === numericId),
    reflections: db.reflections.filter(r => Number(r.student_id) === numericId),
    badges: db.badges.filter(b => Number(b.student_id) === numericId),
    essays: db.essays.filter(e => Number(e.student_id) === numericId),
...
function saveProgress(studentId, chapterId, xpEarned, quizScore = null) {
  ...
  const numericId = student.id;
  const existing = db.progress.find(p => Number(p.student_id) === numericId && p.chapter_id === chapterId);
  if (!existing) {
    db.progress.push({
...
function awardBadge(studentId, badgeId) {
  ...
  const numericId = student.id;
  const hasBadge = db.badges.find(b => Number(b.student_id) === numericId && b.badge_id === badgeId);
  if (!hasBadge) {
    db.badges.push({ student_id: numericId, badge_id: badgeId, earned_at: new Date().toISOString() });
    updateActivity(numericId);
  }
}
